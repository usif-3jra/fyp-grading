...
2. Go to **Email → Domains → Add Domain** → enter `mirror-logic.com`
3. Add the SPF and DKIM records MailerSend provides into Cloudflare DNS (proxy off — grey cloud)
4. Click **Verify** once DNS propagates (~5–15 min)
5. Go to **API Tokens → Generate new token** → select **Full Access**
6. Copy the token → this is `MAILERSEND_API_KEY`
7. Set `SENDER_EMAIL` to your verified sender address (e.g. `noreply@mirror-logic.com`)
...
| `SENDER_EMAIL` | `noreply@mirror-logic.com` | Verified sender email address |
